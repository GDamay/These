...
% \begin{definition}\label{def:gini}
% The Gini index of $S$ is $g(S) = 2\, p_S (1-p_S)$, where $p_S = \frac{1}{|S|} \sum_{(x,y) \in S} y$. The Gini gain of $(j,t)$ on $S$ is:
% \begin{align}
% G(S,j,t) = g(S) - \left(\frac{|S_-|}{|S|} g(S_-) + \frac{|S_+|}{|S|} g(S_+)\right)
% \end{align}
% where $S_-=S[x_j \le t]$ and $S_+=S[x_j > t]$. When $|S|=0$ we define $g(S)=0$ and $G(S,j,t)=0$ for all $(j,t) \in [d] \times \R$.
% \end{definition}
% %When $\scX = \{0,1\}$ we let $S_j^-=S[x_j = 0]$ and $S_j^+=S[x_j = 1]$.
% For all $j \in [d]$ let $G(S,j) = \max_{t\in \R} G(S,j,t)$. Hence, $\argmax_{j}G(S,j)$ is a feature with maximum Gini gain over $S$. Finally, we let $G(S)=\max_{j \in [d]} G(S,j)$.
...
\subsection{Online decision trees}
...
\end{itemize}\todo{Rephrase that whole section}
...
We compare \algo\ against two state-of-the-art algorithms for incremental decision tree learning %mentioned above
