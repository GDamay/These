...
%In section \ref{sec:exp} we conduct an experimental evaluation.
...
Finally, section \ref{sec:conclusion} summarizes our work and discussed interesting directions for future work.\todo{Update this summary}
...
%Let $\alpha$ be the "success" probability of the geometric distribution for the length of the random walk, that is the probability at each step that our random walk terminates given that it didn't terminate before, and $X_0$ and $X_f$ the vertex on which the walker is respectively before the first step and when the walk terminates. We have
%The length of the random walk in the PPR algorithm follows a geometric distribution. Let $\alpha$ be the "success" probability of this distribution, that is the probability at each step that our random walk terminates given that it didn't terminate before, and $X_0$ and $X_f$ the vertex on which the walker is respectively before the first step and after the last step.
Let $\alpha$ be the probability at each step that our random walk terminates given that it didn't terminate before, and $X_0$ and $X_f$ the vertex on which the walker is respectively before the first step and after the last step.
...
\end{equation}
\noindent where $\vect{e_u}$ is the $u$\textsuperscript{th} vector from the canonical basis of $\realset^n$, i.e a vector of which $u$\textsuperscript{th} entry is $1$ and all other entries are $0$. %It can be rewritten equivalently
%\begin{equation}\label{eq:PR_expl}
%    \vect{\pi_u}^\top = \alpha \vect{e_u}^\top + %(1-\alpha) \vect{\pi_u}^\top \vect{P}
%\end{equation}

We call \textit{PPR matrix of the graph}, and we note $\mat{\Pi}$ the matrix of which each line is the PPR vector of the related node. We can see from \eqref{eq:PR_dev} %and \eqref{eq:PR_expl} 
that 

\begin{equation}\label{eq:gen_PR_dev}
    \mat{\Pi} = \alpha\sum_{i=0}^\infty (1-\alpha)^i \mat{P}^i
\end{equation}
%\noindent and it can be rewritten equivalently
%\begin{equation}\label{eq:gen_PR_expl}
%    \mat{\Pi} = \alpha\identity{n} + (1-\alpha) %\mat{\Pi} \mat{P}
%\end{equation}

We call \textit{Reversed PPR vector of a node} the vector $\vect{\gamma_u}$ that contains, for each dimension $v$, the PPR score when the node $v$ is the source node and the node $u$ is the target node, i.e. $\vect{\gamma_u}_v = \vect{\pi_v}_u$.
...
Our method consists of three main parts. First, a truncated Singular Values Decomposition is performed. % on the centered PPR matrix.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
    \Procedure{\textsc{Parfaite}}{$\mat{P}$: stochastic random walk matrix, $m_P$: function that approximate $\mat{\Bar{\Pi}}\vect{v}$ for any $\vect{v}$}
...
%We illustrate this claim with three toy graphs represented in the Figure \ref{fig:graphs_PPR}. The two first graphs represent ideal examples of two natural definitions of a community in a graph: the first one represents a clique in a graph that is a star of cliques, and the second one represent a star in a ring of stars. In both cases, the vertices that connect the community to the rest of the graph are \textit{overlapping} vertices, i.e. vertices that belong to several communities. They have therefore been represented differently.

We illustrate this property with three toy graphs represented in Figure~\ref{fig:graphs_PPR}. The first two graphs provide ideal cases where we can find communities according to two natural community definitions: a) a clique %, where each node is connected to every other node in the clique
...
%Because the PPR vector measures the relevance of the vertices to a source vertex, there can however be some vertices that are central, and hence relevant to the whole graph, an therefore the scores at their dimensions will be high for all reference vertices even when they don't share a community. This is striking on figure \ref{subfig:ppr_mat_ros_graph}, on which we observe that the central nodes in the neighboring cliques have higher scores in the PPR vector of the studied community that other nodes that do belong to the community. The reversed PPR is not affected by that issue.

%We can see on the PPR matrices in subfigures \ref{subfig:ppr_mat_soc_graph}, \ref{subfig:ppr_mat_ros_graph} and \ref{subfig:ppr_mat_toy_graph} that PPR vectors indeed associate larger scores to vertices of the same community as the source node, generating "square" patterns in the matrices.


%We can see on the PPR matrices in subfigures \ref{subfig:ppr_mat_soc_graph}, \ref{subfig:ppr_mat_ros_graph} and \ref{subfig:ppr_mat_toy_graph} that PPR vectors indeed associate larger scores to vertices of the same community as the source node, generating "square" patterns in the matrices.

%We deduce from this fact that if study the scores in the reversed PPR vector of any vertex should be high when the reference (target) vertex shares a community with the source vertex, and low when it is not.

%We illustrate these facts with three toy graphs represented in the Figure \ref{fig:graphs_PPR}. The two first graphs represent ideal examples of two distinct intuitive definitions of a community in a graph: the first one represents a clique in a graph that is a star of cliques, and the second one represent a star in a ring of stars. In both cases, the vertices that connect the community to the rest of the graph are \textit{overlapping} vertices, i.e. vertices that belong to several communities. They have therefore been represented differently.
%
%The last graph is built using the Stochastic Block Model (SBM) so that it has 100 vertices, 300 edges, 4 communities and each pair of vertices of a same community is $100$ times as likely to have an edge between them as a pair of vertices of different communities. 19 out of the 100 vertices belong to two communities each, 7 of which belong to the outlined community.
%
%We can see on the PPR matrices in figures \ref{subfig:ppr_mat_soc_graph}, \ref{subfig:ppr_mat_ros_graph} and \ref{subfig:ppr_mat_toy_graph} that PPR vectors (rows in the matrices) indeed associate larger scores to vertices of the same community as the source node, generating "square" patterns in the matrices. 
...
%However, because the PPR vector measures the relevance of the vertices to a source vertex, there can be some vertices that are reference vertices, and hence relevant to the whole graph. Therefore, the scores in those dimensions will be large for all central vertices even when they do not share communities with other nodes. This is apparent in Figure~\ref{subfig:ppr_mat_ros_graph}, where we observe that the central nodes in the neighboring cliques have higher scores in the PPR vector of the studied community that other nodes that do belong to the community. The reversed PPR is not affected by that issue.
...
Because the SVD only keeps the main patterns of the matrix it decomposes, we expect the result of the SVD to exhibit the typical rows and columns for each community.
%As we saw in section \ref{subsec:back_svd}, the SVD only keeps the main patterns of the matrix it decomposes. %, erasing the small variations.
%Therefore, we expect the result of the SVD to exhibit the typical rows and columns for each community. %, i.e. pattern of the excess of probability to respectively come from and arrive into the community given that we respectively arrived into and came from the community.
We could then interpret these vectors as respectively the belonging of each node to the community and its relevance wrt.  the community.%the nodes of the community. 
...
\subsection{Finding the communities}
...
Choosing the best clustering algorithm for this step is out of the scope of this paper, we simply use the well-known k-means++ algorithm.% and its initialization variant k-means++.
...
%These metrics have in common that they consider an embedding interpretable if the top or bottom entries for each of its dimension approximate the belonging to a ground-truth community.
%We evaluate the results of our algorithms using Betweenness Centrality Importance (BCI) and Closeness Centrality Importance (CCI) proposed by \cite{khoshraftar2021}, as well as the Interpretability Score (IS) proposed by \cite{gogoglou_2019}. We also propose the new Complete Interpretability Score Integrating Priority (CISIP) metric to tackle some weaknesses of the existing ones.
%We evaluate the results of our algorithms using the Interpretability Score (IS) proposed by \cite{gogoglou_2019}. We study the Betweenness Centrality Importance (BCI) and Closeness Centrality Importance (CCI) presented by \cite{khoshraftar2021} but, due the strong limitations of these metrics discussed in section \ref{subsec:bci_cci_introduction} and because of space limitations, we do not include them in our experiments. Finally, we propose the new Complete Interpretability Score Integrating Priority (CISIP) metric to tackle some weaknesses of the existing ones.
...
\subsection{Interpretability score (IS)}
...
%evaluates how well the highest scores of the embedding reflect the belonging to the group,
and a bottom $IS_{bottom(k, g)}$ part defined similarly on the lowest scores.
The article then proposes to aggregate the scores by dimension or ground-truth group using either average or maximum function. %In this paper we will use the maximum. It is also suggested in \cite{gogoglou_2019} that the scores should be aggregated either by embedding dimension or by ground-truth group.
...
\subsection{Betweenness Centrality Importance (BCI) and Closeness Centrality Importance (CCI)}\label{subsec:bci_cci_introduction}
...
%the position of the scores of a group that are missing in the top-$k$ part also influence the interpretability. 
...
%, normalized Betweenness Centrality or Closeness Centrality 
or other functions.
...
%we will use as score for an embedding dimension the sum of its entries for the nodes that belong to the ground-truth community. 
...
%and the embeddings produced by the $U\Sigma$ and $V\Sigma$ multiplications as part of our method. These last two embeddings are included because they are equivalent to the HOPE method when the chosen proximity measure is PPR.
...
Our left embedding greatly outperforms the right one when using CISIP with the PPR smoothing, and this result was expected, as the left embedding is an approximation of the typical PPR vector of each clusters detected. The results are however equivalent between our two embeddings when compared using either the IS or CISIP with the identity smoothing, % (equivalent to no smoothing), 
which both evaluate directly the matching between the embedding and the belonging features, or with the MNB smoothing. This is consistent with our interpretation that the left embedding %correspond respectively to the typical reversed PPR and normal PPR of each community, which can be understood respectively as belonging and importance of the vertices wrt. the community.
...
%so that the dimension of this embedding is identical to the dimension of the \textsc{Parfaite} embeddings.
...
Overall we conclude that \newembLeft and \newembRight do generally perform better than single SVD, i.e. without the clustering and reconstruction steps. %, but that it is wrong in some specific cases. We leave as a future work the study of the rare cases when the clustering reduces interpretability.

\subsection{Is our embedding efficient ?}
%In this paper, we mainly focus on the interpretability of the embedding. This interpretability should however not come at the cost of a too high lost of efficiency in the task the embedding helps solving.
...
\begin{figure}
   \centering
    ...
    \caption{Mean Precision@k of the Link Prediction on 20 test graphs built from the Wikispeedia and Facebook 1912 datasets}
    \label{fig:lp_results}
\end{figure}

%\begin{figure*}
%\centering
%\includegraphics[width=0.3\linewidth]{media/From_ASONAM/LP/LP_results_wikispeedia.png}\label{subfig:lp_wikispeedia}
%\includegraphics[width=0.3\linewidth]{media/From_ASONAM/LP/LP_results_facebook.png}\label{subfig:lp_facebook}
%\caption{Mean Precision@k of the Link Prediction on 20 test graphs built from the Wikispeedia and Facebook 1912 datasets}
%\label{fig:lp_results}
%\end{figure*}
...
%\hline
%Facebook 0 & 0.429 & \textbf{0.429} & 0.143 & 0.138 & 0.074 \\
%\hline
